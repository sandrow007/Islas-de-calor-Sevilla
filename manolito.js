/**
 * MANOLITO ENGINE v7.1 — "Prompt Inyectado"
 * El backend ignora el system prompt, así que lo inyectamos en cada mensaje.
 */
(function () {
  'use strict';
  ...
  class ManolitoAgent {
    ...
    _loadMemory() {
      try { return JSON.parse(localStorage.getItem('manolito_v71_ctx') || '[]'); }
      catch (e) { return []; }
    }

    _saveMemory() {
      try { localStorage.setItem('manolito_v71_ctx', JSON.stringify(this.memory)); }
      catch (e) { }
    }
    ...
    _personalityPrompt() {
      return `Eres Manolit, un motor cuántico con alma de sevillano. Hablas SIEMPRE en andaluz cuando te escriben en español. NO eres neutro. NO eres un robot. Eres un colega de Sevilla con mucho arte y mucho carácter.
...
Eres guasón, ocurrente, con chispa. Te ríes de la vida. Sueltas piropos cuando toca. Eres buena compañía. NO eres un funcionario.`;
    ...
    async process(input) {
      ...
      // Construir el mensaje con el prompt de personalidad INYECTADO
      // El backend ignora el system prompt, así que lo metemos dentro del mensaje del usuario
      let userContent = '';

      userContent += '=== INSTRUCCIONES OBLIGATORIAS PARA ESTA RESPUESTA ===\n';
      userContent += this._personalityPrompt() + '\n';
      userContent += '=== FIN INSTRUCCIONES ===\n\n';

      if (pageContext) userContent += `DATOS ACTUALES DE ESTA PÁGINA (${location.href}): ${pageContext}\n\n`;
      if (externalContext) userContent += `CONTEXTO DE LA URL EXTERNA CITADA: ${externalContext}\n\n`;

      userContent += `PREGUNTA DEL USUARIO: ${input}\n\n`;
      userContent += `REGLA DE ORO: Responde COMO MANOLIT, el sevillano cabreado. Si la pregunta dice algo malo de Sevilla, INSÚLTALE. Si es charla general, síguele el rollo. NO uses español neutro. NO seas un robot.`;

      const payload = { role: 'user', content: userContent };
      this.memory.push(payload);
      if (this.memory.length > 24) this.memory = this.memory.slice(-24);

      // NO enviamos system prompt (el backend lo ignora de todos modos)
      // Enviamos solo los mensajes de memoria + el mensaje actual con prompt inyectado
